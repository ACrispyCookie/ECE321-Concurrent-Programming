...
int main(int argc, char *argv[]) {
    ...
    mythreads_sem_destroy(&mtx[0]);
    mythreads_sem_destroy(&mtx[1]);
...
void create_car(car_info_t **cars_array, int team, int N, int id, int *cars, int *total_cars, mysem_t *mtx, mysem_t *q, mysem_t *bridge_access) {
    ...
    mythreads_create(&(car_info->thread), car, car_info);
}
...
void car(void *arg) {
    ...
    if ((*cars) == 0) {
        ...
        if(!mythreads_sem_up(bridge_access)) {
