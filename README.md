...
Coursework repository for ECE321 Concurrent Programming. The projects are implemented primarily in C and cover pthread-based synchronization, custom semaphore abstractions, monitor-style coordination, coroutines, and user-level threads.
...
`make test` runs representative inputs for the main executable assignments. It verifies that:

- the FIFO file-transfer assignments reproduce the input file exactly,
- the external mergesort output is sorted,
- semaphore and monitor-based simulations complete on selected input cases,
- the coroutine and user-level-thread runtimes can run their dependent workloads,
- the Java assignment compiles and starts successfully.

The validation checks are intended as a quick regression check. They are not a replacement for the full set of assignment experiments and performance measurements described in the coursework handouts.
